...
type MutableAssistantContent =
  | MutableTextContent
  | MutableThinkingContent
  | MutableToolCall;
...
function toMutableAssistantMessage(
...
function toAssistantMessageSnapshot(
      ...
      for (const block of msg.content) {
        if (block.type === "thinking") {
          ...
          output.push({
            ...
            status: "completed",
            id: msgId,
            ...(parsedSignature?.phase !== undefined
  ...
  for await (const event of openaiStream) {
    if (event.type === "response.output_item.added") {
      ...
      if (
        ...
        currentBlock.arguments = decodeToolArguments(
          model.provider,
          currentBlock.partialJson,
        );
      }
      ...
      if (item.type === "reasoning" && currentBlock?.type === "thinking") {
        ...
        const toolCallBlock =
          currentBlock?.type === "toolCall" ? currentBlock : undefined;

        if (toolCallBlock) {
          toolCallBlock.arguments =
            toolCallBlock.partialJson !== undefined &&
            toolCallBlock.partialJson.trim() !== ""
