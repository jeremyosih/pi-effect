...
export class AuthMissing extends S.TaggedErrorClass<AuthMissing>("AuthMissing")("AuthMissing", {
  provider: S.String,
}) {}
...
const failMissingAuth = (provider: Provider) => new AuthMissing({ provider });

const explicitApiKeyOption = (apiKey: string | undefined): Option.Option<EnvApiKey> =>
  apiKey && apiKey.length > 0
    ? Option.some(Redacted.make(apiKey, { label: "explicitApiKey" }))
    : Option.none();

const hasVertexAdcCredentials = Effect.fn("AuthResolver.hasVertexAdcCredentials")(function* () {
  ...
  const gacPath = yield* googleApplicationCredentialsConfig
    .asEffect()
    .pipe(Effect.orElseSucceed(() => Option.none<string>()));

  if (Option.isSome(gacPath)) {
    const exists = yield* maybeFs.value
      .exists(gacPath.value)
  ...
  const exists = yield* maybeFs.value
    .exists(
      maybePath.value.join(_homedir(), ".config", "gcloud", "application_default_credentials.json"),
    )
    .pipe(Effect.orElseSucceed(() => false));
...
const resolveOAuth = Effect.fn("AuthResolver.resolveOAuth")(function* (provider: Provider) {
  ...
  return yield* maybeOAuthResolver.value
    .resolveAuth(provider)
    .pipe(Effect.catchTag("OAuthUnavailable", () => Effect.fail(failMissingAuth(provider))));
});

const resolveAuth = Effect.fn("AuthResolver.resolveAuth")(function* (input: ResolveAuthInput) {
  ...
  const envAuth = yield* envAuthConfig(input.provider)
    .asEffect()
    .pipe(Effect.orElseSucceed(() => Option.none<EnvAuth>()));
  ...
  if (input.provider === "google-vertex") {
    ...
    const project = yield* googleVertexProjectConfig
      .asEffect()
      .pipe(Effect.orElseSucceed(() => Option.none<string>()));
    const location = yield* googleVertexLocationConfig
      .asEffect()
      .pipe(Effect.orElseSucceed(() => Option.none<string>()));
...
const resolveApiKey = Effect.fn("AuthResolver.resolveApiKey")(function* (input: ResolveAuthInput) {
  const auth = yield* resolveAuth(input);

  return auth === AUTHENTICATED ? yield* failMissingAuth(input.provider) : auth;
});

export class AuthResolver extends ServiceMap.Service<
  AuthResolver,
  {
    readonly resolveAuth: (input: ResolveAuthInput) => Effect.Effect<EnvAuth, AuthMissing>;
    readonly resolveApiKey: (input: ResolveAuthInput) => Effect.Effect<EnvApiKey, AuthMissing>;
  }
